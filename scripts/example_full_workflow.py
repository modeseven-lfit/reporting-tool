...
def simulate_jenkins_matching(
    ...
    project: str,
    simulated_jenkins_jobs: Set[str]
) -> Dict:
    """
    ...
    logger.info(f"  Found in Jenkins: {len(matched)} ({len(matched)/len(resolved_jobs)*100:.1f}%)")
...
def compare_approaches(results: Dict) -> None:
    ...
    logger.info("\nCode Complexity:")
    logger.info(f"  Fuzzy Matching:    ~70 LOC, complex scoring")
    logger.info(f"  CI-Management:     ~15 LOC, simple exact matching")
    logger.info(f"  Reduction:         -79%")
    
    logger.info("\nMaintainability:")
    logger.info(f"  Fuzzy Matching:    Requires tuning, many edge cases")
    logger.info(f"  CI-Management:     No tuning needed, self-documenting")
    
    logger.info("\nExtensibility:")
    logger.info(f"  Fuzzy Matching:    Manual updates for new job types")
    logger.info(f"  CI-Management:     Automatic support for new job types")
