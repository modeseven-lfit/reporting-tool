...
from src.performance.memory import (
    LazyLoader,
    LazyProxy,
    MemoryMonitor,
    MemoryOptimizer,
...
class TestMemoryMonitor:
    ...
    def test_snapshot_tracking(self):
        ...
        snapshot1 = monitor.snapshot("op1")
        snapshot2 = monitor.snapshot("op2")
    ...
    def test_alert_threshold(self):
        ...
        with patch.object(monitor, "get_current_memory", return_value=100.0):
            with patch("src.performance.memory.logger") as mock_logger:
    ...
    def test_lazy_loading_performance(self):
        ...
        lazy_objects = [
            optimizer.create_lazy(
                lambda i=i: {"data": [j for j in range(1000)]},
                f"lazy_{i}",
            )
            for i in range(100)
        ]
