...
class TestAssertGitOperation:
    ...
    def test_failed_git_operation(self, temp_git_repo):
        ...
        with pytest.raises(AssertionError, match="Git operation failed"):
            with assert_git_operation("failing operation", temp_git_repo):
    ...
    def test_without_repository_path(self):
        ...
        with pytest.raises(AssertionError, match="Git operation failed"):
            with assert_git_operation("test operation"):
...
class TestAssertTestOperation:
    ...
    def test_failed_operation(self):
        """Test context manager with failed operation."""
        with pytest.raises(AssertionError, match="Test operation failed"):
            with assert_test_operation("failing operation"):
...
class TestIntegrationScenarios:
    ...
    def test_nested_context_managers(self, temp_git_repo):
        ...
        with assert_test_operation("nested operation test"):
            # Outer operation
            with assert_git_operation("outer git operation", temp_git_repo):
                # Inner operation
    ...
    def test_error_propagation_chain(self, temp_git_repo):
        ...
        try:
            with assert_test_operation("outer"):
                with assert_git_operation("inner", temp_git_repo):
                    raise original_error
            pytest.fail("Should have raised AssertionError")
        except AssertionError as e:
