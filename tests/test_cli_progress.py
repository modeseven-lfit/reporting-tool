...
class TestProgressIndicator(unittest.TestCase):
    ...
    @patch("src.cli.progress.TQDM_AVAILABLE", False)
    ...
    @patch("src.cli.progress.TQDM_AVAILABLE", True)
    @patch("src.cli.progress.tqdm")
...
class TestProgressBarContextManager(unittest.TestCase):
    ...
    def test_with_manual_total(self):
        ...
        stderr = io.StringIO()
        with redirect_stderr(stderr):
            with progress_bar(total=5, desc="Test", disable=False) as pbar:
                for i in range(5):
                    pbar.update(1)
...
class TestProgressIntegration(unittest.TestCase):
    ...
    def test_sequential_progress(self):
        ...
        stderr = io.StringIO()
        with redirect_stderr(stderr):
            with ProgressIndicator(total=len(items), desc="Sequential") as progress:
                for item in items:
