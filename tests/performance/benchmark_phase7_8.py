...
def benchmark_concurrent_rendering(concurrency_levels: list[int] = [1, 5, 10]) -> dict[int, float]:
    """
    ...
    for level in concurrency_levels:
        ...
        with ThreadPoolExecutor(max_workers=level) as executor:
            futures = []
            for i in range(level * 5):  # 5x concurrency level tasks
                renderer = ModernReportRenderer(config, logger)
...
def benchmark_adaptive_pool(task_counts: list[int] = [10, 50, 100]) -> dict[int, float]:
    """
