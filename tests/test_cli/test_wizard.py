...
class TestCreateConfigFromTemplate:
    ...
    def test_create_standard_config(self, tmp_path):
        ...
        output_path = tmp_path / "config.yaml"
        result = create_config_from_template("test-project", "standard", str(output_path))
    ...
    def test_create_full_config(self, tmp_path):
        ...
        output_path = tmp_path / "config.yaml"
        result = create_config_from_template("test-project", "full", str(output_path))
...
class TestConfigurationWizard:
    ...
    def test_wizard_minimal_flow(self, mock_print, mock_input, tmp_path):
        ...
        with patch.object(wizard, "_save_configuration", return_value=str(output_path)):
            with patch.object(wizard, "_print_next_steps"):
                result = wizard.run(str(output_path))
...
class TestRunWizard:
    ...
    def test_run_wizard_calls_wizard(self, mock_wizard_class, tmp_path):
        ...
        output_path = tmp_path / "config.yaml"
        result = run_wizard(str(output_path))
