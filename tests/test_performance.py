...
class TestIntegrationScenarios:
    ...
    def test_full_profiling_workflow(self):
        ...
        git_ops = sum(1 for key in aggregated.keys() if key.startswith("git:"))
        analysis_ops = sum(1 for key in aggregated.keys() if key.startswith("analysis:"))
...
class TestEdgeCases:
    ...
    def test_timer_exception_propagation(self):
        ...
        with pytest.raises(RuntimeError, match="Test error"):
            with profiler.track_operation("failing_op", category="test"):
